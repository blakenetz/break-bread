	...
	validateInput(obj) {
		for (key in obj) {
			...
			if (
				...
				(key == "password" &&
					val.length < 8 &&
					passwordBlackList.indexOf(val) > -1) ||
		...
		if (this.props.mode == "signup") {
			await Auth.signUp({
				...
				.catch(err => {
					...
					this.props.updateFormState({
						mode: "login",
						message: `Well crud... something bad happened: ${
							err.code
						}`
					});
				});
		...
		return (
			<Fragment>
				{this.inputs.map((input, i) => {
					return (
						<AuthInput
							...
							hasError={
								input.hasError ||
								this.state.errors.indexOf(input.name) > -1
							}
				...
				{this.state.mode !== "verify" ? (
					<Text
						onPress={() =>
							this.props.updateFormState({ mode: "re-verify" })
						}
				...
				<AuthMenu handlePress={this.props.updateFormState} />
			</Fragment>
		);
	}
}
